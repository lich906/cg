...
MorphingParaboloid::MorphingParaboloid(const glm::vec3& position)
	: m_mesh(GenerateVertices(), GenerateIndices())
	, m_transform(glm::translate(glm::mat4(1.0f), position))
{
}

void MorphingParaboloid::OnUpdate(core::Timestep ts)
{
	m_t = glm::cos(m_x += ts * MORPH_SPEED) * 0.5f + 0.5f;
}

void MorphingParaboloid::OnDraw(gfx::Program& prog)
{
...
std::vector<gfx::Vertex> MorphingParaboloid::GenerateVertices() const
{
...
std::vector<GLuint> MorphingParaboloid::GenerateIndices() const
{
...
glm::vec3 MorphingParaboloid::GetNormalMultiplier() const
{
