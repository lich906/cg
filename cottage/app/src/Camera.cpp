...
static constexpr float ZOOM_STEP = 10.0f;
...
bool Camera::OnMouseMoved(core::event::MouseMovedEvent& event)
{
	...
	if (m_dragging)
	{
		...
		m_yawAngle -= delta.x * ROTATION_SENSITIVITY;
		m_pitchAngle -= delta.y * ROTATION_SENSITIVITY;
...
{
	m_zoomLevel -= event.GetYOffset() * ZOOM_STEP;
	UpdateViewMatrix();
	return true;
}
